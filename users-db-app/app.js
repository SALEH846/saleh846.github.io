...
const getUsers = () => {
    axios.get('https://users-db-app.herokuapp.com/users')
        .then(response => {
...
const getUserFromDB = userID => {
    axios.get(`https://users-db-app.herokuapp.com/user/${userID}`)
        .then(response => {
...
const submitData = (name, email, address) => {
    axios.post('https://users-db-app.herokuapp.com/user', {name, email, address})
        .then(response => {
            getUser();
        })
        .catch(err => {
...
const editUserDB = (id, name, email, address) => {
    if (name) {
        axios.put(`https://users-db-app.herokuapp.com/user/${id}`, {name})
            .then(response => getUsers())
    }
    if (email) {
        axios.put(`https://users-db-app.herokuapp.com/user/${id}`, {email})
            .then(response => getUsers())
    }
    if (address) {
        axios.put(`https://users-db-app.herokuapp.com/user/${id}`, {address})
            .then(response => getUsers())
    }
}
...
const deleteUser = () => {
    ...
    if (userID.value) {
        axios.delete(`https://users-db-app.herokuapp.com/user/${userID.value}`)
            .then(response => getUsers())
    }
    userID.value = "";
}